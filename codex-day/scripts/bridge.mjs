...
class Session {
  ...
  async ensure(){
    ...
    try{const result=await this.send('Runtime.evaluate',{contextId:this.context,expression:'globalThis.__codexDayUI?.verify()',returnByValue:true});
      if(result.exceptionDetails||result.result?.value?.version!=='0.3.2')await this.inject();
...
async function main(){
  ...
  while(!stopping){try{
    ...
    await fs.writeFile(path.join(base,'connection.json'),JSON.stringify({phase:'connected',windows:sessions.size,version:'0.3.2',checkedAt:new Date().toISOString()}),{mode:0o600});
    previous='';
  }catch(e){for(const session of sessions.values())session.ws.close();sessions.clear();await fs.writeFile(path.join(base,'connection.json'),JSON.stringify({phase:'waiting-for-extension-launch',version:'0.3.2',checkedAt:new Date().toISOString()}),{mode:0o600}).catch(()=>{});const message='Waiting for Codex local extension connection.';if(previous!==message){console.log(message);previous=message}}
  await sleep(5000)}
}
